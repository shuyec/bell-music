...
class AudioPlayer {
  ...
  AudioPlayer({
    ...
    _processingStateSubject.addStream(
        playbackEventStream.map((event) => event.processingState).distinct().handleError((Object err, StackTrace stackTrace) {/* noop */}));
    _bufferedPositionSubject.addStream(
        playbackEventStream.map((event) => event.bufferedPosition).distinct().handleError((Object err, StackTrace stackTrace) {/* noop */}));
    _icyMetadataSubject
    ...
    _androidAudioSessionIdSubject.addStream(
        playbackEventStream.map((event) => event.androidAudioSessionId).distinct().handleError((Object err, StackTrace stackTrace) {/* noop */}));
...
class ConcatenatingAudioSource extends AudioSource {
  ...
  Future<void> removeRange(int start, int end) async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform).concatenatingRemoveRange(
          ConcatenatingRemoveRangeRequest(id: _id, startIndex: start, endIndex: end, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> move(int currentIndex, int newIndex) async {
    ...
    if (_player != null) {
      ...
      await (await _player!._platform).concatenatingMove(
          ConcatenatingMoveRequest(id: _id, currentIndex: currentIndex, newIndex: newIndex, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
...
class ResolvingAudioSource extends StreamAudioSource {
  ...
  ResolvingAudioSource({required this.uniqueId, required this.resolveSoundUrl, this.headers, dynamic tag}) : super(tag: tag);
  ...
  Future<StreamAudioResponse> request([int? start, int? end]) async {
    ...
    if (soundUrl == null) {
      return StreamAudioResponse(sourceLength: null, contentLength: null, offset: null, stream: const Stream.empty(), contentType: '');
    }
...
class LockCachingAudioSource extends StreamAudioSource {
  ...
  Future<HttpClientResponse> _fetch() async {
    ...
    subscription = response.listen((data) async {
      ...
      sink.add(data);
      final readyRequests =
          _requests.where((request) => !originSupportsRangeRequests || request.start == null || (request.start!) < _progress).toList();
...
_ProxyHandler _proxyHandlerForSource(StreamAudioSource source) {
  Future<void> handler(_ProxyHttpServer server, HttpRequest request) async {
    ...
    if (rangeRequest != null && sourceResponse.offset != null) {
      final range =
          _HttpRangeResponse(sourceResponse.offset!, sourceResponse.offset! + sourceResponse.contentLength! - 1, sourceResponse.sourceLength);
